...
import { ShieldAlert, ArrowLeft, Plus, UploadCloud, Video, HelpCircle, MessageSquare, Trash2, Save, CheckCircle2, Edit, FolderPlus, GripVertical } from "lucide-react";
...
export default function CourseEditorPage({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  if (isAddingTopic) {
    ...
    return (
      <div className="w-full pb-12  animate-in fade-in">
        <button onClick={() => setIsAddingTopic(false)} className="inline-flex items-center text-sm text-zinc-400 hover:text-cyan-400 mb-6 transition-colors">
        ...
        <Card className="p-6 sm:p-8 space-y-12">
          {/* 1. Basic Info */}
          <section>
            <h2 className="text-lg font-bold text-white mb-6 border-b border-zinc-800 pb-2">1. Topic Details</h2>
          ...
          {/* 2. Video Upload */}
          <section>
            <h2 className="text-lg font-bold text-white mb-6 border-b border-zinc-800 pb-2 flex items-center">
              ...
              {mcqs.map((mcq, qIdx) => (
                <div key={qIdx} className="bg-zinc-950 border border-zinc-800 p-6 rounded-xl relative">
                  ...
                  <div className="mb-4 pr-8">
                    <label className="block text-sm font-medium text-zinc-300 mb-1.5">Question {qIdx + 1}</label>
                    <Input 
                      value={mcq.question} 
          ...
          <div className="flex justify-end gap-4 pt-6 border-t border-zinc-800">
            <button onClick={() => setIsAddingTopic(false)} className="px-6 py-3 bg-zinc-800 hover:bg-zinc-700 text-white font-medium rounded-lg transition-colors">
              Cancel
            </button>
            <button onClick={handleSaveTopic} className="inline-flex items-center justify-center px-6 py-3 bg-cyan-400 hover:bg-cyan-500 text-zinc-950 font-bold font-medium rounded-lg transition-colors shadow-[0_0_20px_rgba(8,145,178,0.3)] cursor-pointer">
  ...
  return (
    <div className="w-full pb-12  animate-in fade-in">
      ...
      {isAddingModule && (
        <div className="bg-zinc-900 border border-cyan-900/50 rounded-xl p-6 mb-8 shadow-lg">
          <h3 className="text-base font-semibold text-white mb-4">Create New Module</h3>
        ...
        {modules.length > 0 ? modules.map((module, mIdx) => {
          ...
          return (
            <div key={module.id} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden shadow-sm">
              <div className="bg-zinc-950/50 p-4 sm:p-5 flex items-center justify-between border-b border-zinc-800">
              ...
              <div className="p-4 sm:p-6 space-y-3 bg-zinc-900">
                {moduleTopics.length > 0 ? moduleTopics.map((topic, tIdx) => (
                  <div key={topic.id} className="group bg-zinc-950 border border-zinc-800 rounded-lg p-4 flex items-center justify-between hover:border-zinc-700 transition-colors">
                    <div className="flex items-center gap-4">
                      <div className="w-8 h-8 rounded-full bg-zinc-900 border border-zinc-800 text-zinc-500 flex items-center justify-center font-medium text-sm">
                        {tIdx + 1}
                      </div>
                      <div>
                        <h3 className="text-base font-semibold text-zinc-200 group-hover:text-cyan-400 transition-colors">{topic.title}</h3>
                  ...
                  <div className="text-center p-6 text-sm text-zinc-500 italic border border-dashed border-zinc-800 rounded-lg">
                ...
                <button 
                  ...
                  className="w-full mt-3 flex items-center justify-center p-3 border border-dashed border-zinc-700 hover:border-cyan-800 hover:bg-cyan-950/20 text-zinc-400 hover:text-cyan-400 rounded-lg transition-colors text-sm font-medium"
                >
          ...
          <div className="p-16 text-center border-2 border-dashed border-zinc-800 rounded-xl">
            ...
            <h3 className="text-base font-semibold text-white mb-2">Your curriculum is empty</h3>
