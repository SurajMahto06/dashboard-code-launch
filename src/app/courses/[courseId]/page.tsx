...
export default function CourseSyllabusPage({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  return (
    <div className="w-full pb-12 ">
      <Link href="/courses" className="inline-flex items-center text-sm text-zinc-400 hover:text-cyan-400 mb-8 transition-colors">
        ...
        {!isEnrolled && (
          <div className="mt-6 p-4 bg-zinc-900 border border-zinc-800 rounded-xl flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-zinc-400">Enroll to get full access to all curriculum materials.</p>
            </div>
      ...
      <div className="space-y-4">
        <h2 className="text-lg font-bold text-white mb-6">Course Syllabus</h2>
            ...
            <div key={module.id} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden shadow-sm">
              ...
              >
                <div className="flex flex-col items-start">
                  <span className="text-xs font-bold text-cyan-500 uppercase tracking-wider mb-1">Module {module.order}</span>
                  <h3 className="text-base font-semibold text-white text-left">{module.title}</h3>
                </div>
                    ...
                    <div key={topic.id} className="group relative">
                      <div className="p-4 sm:p-5 flex items-center justify-between hover:bg-zinc-800/30 transition-colors">
                        <div className="flex items-start gap-4">
                          <div className="w-8 h-8 rounded-full bg-zinc-800 flex items-center justify-center text-zinc-400 shrink-0 font-medium text-sm border border-zinc-700">
                            {tIdx + 1}
                          </div>
                          <div>
                            <h4 className={`font-semibold ${isEnrolled ? 'text-white group-hover:text-cyan-400' : 'text-zinc-300'} transition-colors`}>
                              {topic.title}
                            </h4>
                        ...
                        {isEnrolled ? (
                          <Link 
                            href={`/topic/${topic.id}`}
                            className="px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-white text-sm font-medium rounded-lg transition-colors absolute inset-0 sm:static sm:inset-auto opacity-0 sm:opacity-100 group-hover:opacity-100 flex items-center justify-center sm:justify-end"
                          >
