...
export default function NewIssueCertificatePage() {
  ...
  const handleDownloadPDF = async () => {
    if (!issuedId) return;
    const student = mockUsersDB.find(u => u.id === selectedStudentId);
        ...
        {issuedId ? (
          ...
          <form onSubmit={handleIssue} className="space-y-8">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setSelectedStudentId(""); // Reset student on course change
                  }}
                  className="w-full px-4 py-2.5 bg-zinc-950 border border-zinc-800 rounded-lg text-[13px] text-white focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all appearance-none cursor-pointer"
                >
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2.5 bg-zinc-950 border border-zinc-800 rounded-lg text-[13px] text-white focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all appearance-none disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    type="month"
                    name="startDate"
                    value={formData.startDate.substring(0, 7)} // format YYYY-MM
                    onChange={handleChange}
                    className="w-full px-4 py-2.5 bg-zinc-950 border border-zinc-800 rounded-lg text-[13px] text-white placeholder-zinc-500 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all [color-scheme:dark]"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    type="month"
                    name="endDate"
                    value={formData.endDate.substring(0, 7)}
                    onChange={handleChange}
                    className="w-full px-4 py-2.5 bg-zinc-950 border border-zinc-800 rounded-lg text-[13px] text-white placeholder-zinc-500 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all [color-scheme:dark]"
                  />
                </div>
              ...
              <div>
                ...
                <input
                  ...
                  type="month"
                  name="dateOfIssue"
                  value={formData.dateOfIssue.substring(0, 7)}
                  onChange={handleChange}
                  className="w-full px-4 py-2.5 bg-zinc-950 border border-zinc-800 rounded-lg text-[13px] text-white placeholder-zinc-500 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all [color-scheme:dark]"
                />
              </div>
            ...
            <div className="pt-6 border-t border-zinc-800 flex justify-end">
              <button
                type="submit"
                disabled={isIssuing || !selectedCourseId || !selectedStudentId}
