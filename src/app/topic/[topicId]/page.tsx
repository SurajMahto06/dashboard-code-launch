...
export default function TopicPage({ params }: { params: Promise<{ topicId: string }> }) {
  ...
  if (user?.role === "student" && !user.enrolledCourseIds?.includes(topic.courseId)) {
    return (
      <div className="max-w-3xl mx-auto p-12 text-center bg-red-950/20 border border-red-900 rounded-2xl mt-12">
    ...
    <div className="max-w-6xl mx-auto pb-12 w-full">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-2">
          <span className="text-xs font-semibold text-cyan-400 uppercase tracking-wider">{topic.module}</span>
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="space-y-4">
          ...
          <Link href={`/topic/${topic.id}/mcq`} className="flex items-center p-4 bg-zinc-900 border border-zinc-800 rounded-xl hover:border-cyan-800 transition-colors group">
            ...
            <div>
              <h3 className="font-bold text-white group-hover:text-cyan-400 transition-colors">Take the Quiz</h3>
          ...
          <Link href={`/topic/${topic.id}/interview`} className="flex items-center p-4 bg-zinc-900 border border-zinc-800 rounded-xl hover:border-cyan-800 transition-colors group">
            ...
            <div>
              <h3 className="font-bold text-white group-hover:text-cyan-400 transition-colors">Interview Questions</h3>
